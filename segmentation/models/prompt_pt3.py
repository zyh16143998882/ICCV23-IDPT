...
class TransformerPromptEncoderLayerI(nn.Module):
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., num_group=0, prompt_layer=11):
        super().__init__()
        ...
        args = Args()
        # self.dgcnn_cls3 = DGCNNView(args, self.trans_dim)
        self.dgcnn_cls3 = DGCNNViewMLP(args, self.trans_dim)
        self.dgcnn_cls7 = DGCNNViewMLP(args, self.trans_dim)
        self.dgcnn_cls11 = DGCNNViewMLP(args, self.trans_dim)
        ...
        for _, block in enumerate(self.blocks):
            ...
            if _ in fetch_idx:
                ...
                if _ == 3:
                    ...
                    cls = self.dgcnn_cls11

                prompt = cls(x.permute(0, 2, 1))
                x = torch.cat((prompt, x), dim=1)
                pos = torch.cat((pos_prompt, pos), dim=1)
...
class get_model(nn.Module):
    def __init__(self, cls_dim):
        ...
        self.prompt_cls_pos = nn.Parameter(torch.randn(1, 1, self.trans_dim))
        ...
        # self.convs1_cls = nn.Conv1d(2240, 512, 1)
        ...
        trunc_normal_(self.prompt_cls_pos, std=.02)
    ...
    def forward(self, pts, cls_label):
        ...
        prompt_cls_pos = self.prompt_cls_pos.expand(group_input_tokens.size(0), -1, -1)
        ...
        prompt_feature = torch.cat((feature_list[0][:,:,:1],feature_list[1][:,:,:1],feature_list[2][:,:,:1]), dim=1).repeat(1, 1, N)
        prompt_feature_2 = torch.cat((feature_list[0][:,:,1:2],feature_list[1][:,:,1:2],feature_list[2][:,:,1:2]), dim=1).repeat(1, 1, N)
        x = torch.cat((feature_list[0][:,:,-self.num_group:],feature_list[1][:,:,-self.num_group:],feature_list[2][:,:,-self.num_group:]), dim=1) #1152
        ...
        x_global_feature = torch.cat((prompt_feature, prompt_feature_2, cls_label_feature), 1)  # 1152*2 + 64
        # x_global_feature = torch.cat((x_max_feature, x_avg_feature, prompt_feature, cls_label_feature), 1)  # 1152*3 + 64
