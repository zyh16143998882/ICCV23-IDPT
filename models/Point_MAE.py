...
from .dgcnn import DGCNNView, DGCNNViewLight, DGCNNViewLight2, DGCNNViewMLP
...
class BlockAdapter(nn.Module):
    ...
    def forward(self, x):
        ...
        adpt = self.adapter_downsample_cls(x.permute(0,2,1))
        adpt = self.adapter_act_fn_cls(adpt)
        adpt = self.adapter_upsample_cls(adpt)
        x = adpt.permute(0,2,1) + x
...
class TransFormerLayerView(nn.Module):
    ...
    def forward(self, x,pos):
        ...
        x = self.blocks(x.permute(0,2,1), pos)
...
class TransformerPromptEncoderLayerI(nn.Module):
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        if prompt_module == "transformer":
            self.cls = TransFormerLayerView(trans_dim=embed_dim,depth=1,num_heads=num_heads,dpr=drop_path_rate)
            ...
            args = Args()
            self.cls = DGCNNView(args, self.trans_dim)
            ...
            args = Args()
            self.cls = DGCNNViewLight(args, self.trans_dim)
            ...
            args = Args()
            self.cls = DGCNNViewLight2(args, self.trans_dim)
            ...
            args = Args()
            self.cls = DGCNNViewMLP(args, self.trans_dim)


    def forward(self, x, pos):
        y = None
        pos_prompt = pos[:,1:2,:]
        pos = torch.cat((pos[:,:1, :], pos[:,-self.num_group:,:]),dim=1)
        for _, block in enumerate(self.blocks):
...
class PointTransformerAdapter(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        dpr = [x.item() for x in torch.linspace(0, self.drop_path_rate, self.depth)]
        self.blocks = TransformerEncoderAdapter(
            embed_dim=self.trans_dim,
            depth=self.depth,
            drop_path_rate=dpr,
        )
...
@MODELS.register_module()
class PointTransformerRaw(nn.Module):
    def __init__(self, config, **kwargs):
        super().__init__()
        self.config = config
...
class PointTransformerPromptDeep(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        self.cls_prompt_list = nn.ParameterList([
            nn.Parameter(torch.zeros(1, self.prompt_nums, self.trans_dim))
            for i in range(self.depth)])
        self.cls_prompt_pos_list = nn.ParameterList([
            nn.Parameter(torch.randn(1, self.prompt_nums, self.trans_dim))
            for i in range(self.depth)])
        ...
        for i in range(self.depth):
            trunc_normal_(self.cls_prompt_list[i], std=.02)
            trunc_normal_(self.cls_prompt_pos_list[i], std=.02)
    ...
    def forward(self, pts):
        ...
        x = self.blocks(x, pos, self.cls_prompt_list, self.cls_prompt_pos_list)
        x = self.norm(x)

        concat_f = torch.cat([x[:, 0], x[:, 1:self.num_group+1,:].max(1)[0]], dim=-1)
        ret = self.cls_head_finetune(concat_f)
        return ret